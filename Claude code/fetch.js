...
async function fetchIndicesAndMetals() {
  // Each AV call costs 1 of our 25 daily free requests — use carefully
  const [sp500, dax, ftse, nikkei, silver] = await Promise.allSettled([
    safe('AV: S&P 500',  () => avGlobalQuote('SPY')),    // ETF proxy for S&P 500
    safe('AV: DAX',      () => avGlobalQuote('EWG')),    // ETF proxy for DAX
    safe('AV: FTSE',     () => avGlobalQuote('ISF.LON')),
    safe('AV: Nikkei',   () => avGlobalQuote('EWJ')),    // ETF proxy for Nikkei
    safe('AV: Silver',   () => avGlobalQuote('SLV')),    // ETF proxy for silver
  ]);
...
async function fetchMacro() {
  const [treasury10y, treasury2y, cpi, fedFunds, unrate] = await Promise.allSettled([
  ...
  const yieldSpread = (t10 && t2)
    ? { value: round(t10.value - t2.value, 4), date: t10.date }
    : null;
...
async function fetchGeopolitical() {
  ...
  const url = `https://api.gdeltproject.org/api/v2/doc/doc?query=${query}&mode=ArtList&maxrecords=50&format=json&timespan=WEEK`;
...
async function main() {
  ...
  const existing = await safe('Read Gist', readGist);
  ...
  const narrative = generateNarrative({ fearGreed, macro, metals: { gold: goldPrice }, news });
  ...
  const snapshot = {
    ...
    geopolitical: geo ?? existing?.geopolitical ?? null,
